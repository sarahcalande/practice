...
## Rails Backend
This directory contains the back-end for today’s challenge. It is built with rails - something we should all be familiar with after mod 2! Unlike your rails projects, however, this app is missing the front-end. That’s where you come in!

To run this backend, enter the following commands:


   ``` $ cd mod3_backend - (make sure that you've cd'd into the folder containing
   the rails app, otherwise the following commands won't work!)
   $ bundle install
   $ rails db:drop db:create db:migrate db:seed
   $ rails s
   ```

Once your migrations are run, database is seeded, and server is running, you can direct fetch calls to http://localhost:3000/
